trigger "query" "detect_and_correct_compute_disks_with_incorrect_tags" {
  ...
  schedule = var.compute_disks_with_incorrect_tags_trigger_schedule
  database = var.database
  sql      = local.compute_disks_with_incorrect_tags_query
...
variable "compute_disks_tag_rules" {
  type = object({
...
locals {
  compute_disks_tag_rules = {
    ...
    update_values = merge(local.base_tag_rules.update_values, try(var.compute_disks_tag_rules.update_values, {}))
  }
}

locals {
  compute_disks_update_keys_override   = join("\n", flatten([for key, patterns in local.compute_disks_tag_rules.update_keys : [for pattern in patterns : format("      when key %s '%s' then '%s'", (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? element(split(":", pattern), 0) : "="), (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? join(":", slice(split(":", pattern), 1, length(split(":", pattern)))) : pattern), key)]]))
  compute_disks_remove_override        = join("\n", length(local.compute_disks_tag_rules.remove) == 0 ? ["      when new_key like '%' then false"] : [for pattern in local.compute_disks_tag_rules.remove : format("      when new_key %s '%s' then true", (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? element(split(":", pattern), 0) : "="), (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? join(":", slice(split(":", pattern), 1, length(split(":", pattern)))) : pattern))])
  compute_disks_remove_except_override = join("\n", length(local.compute_disks_tag_rules.remove_except) == 0 ? ["      when new_key like '%' then true"] : flatten([[for key in keys(merge(local.compute_disks_tag_rules.add, local.compute_disks_tag_rules.update_keys)) : format("      when new_key = '%s' then true", key)], [for pattern in local.compute_disks_tag_rules.remove_except : format("      when new_key %s '%s' then true", (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? element(split(":", pattern), 0) : "="), (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? join(":", slice(split(":", pattern), 1, length(split(":", pattern)))) : pattern))]]))
  compute_disks_add_override           = join(",\n", length(keys(local.compute_disks_tag_rules.add)) == 0 ? ["      (null, null)"] : [for key, value in local.compute_disks_tag_rules.add : format("      ('%s', '%s')", key, value)])
  compute_disks_update_values_override = join("\n", flatten([for key in sort(keys(local.compute_disks_tag_rules.update_values)) : [flatten([for new_value, patterns in local.compute_disks_tag_rules.update_values[key] : [contains(patterns, "else:") ? [] : [for pattern in patterns : format("      when new_key = '%s' and value %s '%s' then '%s'", key, (length(split(": ", pattern)) > 1 && contains(local.operators, element(split(": ", pattern), 0)) ? element(split(": ", pattern), 0) : "="), (length(split(": ", pattern)) > 1 && contains(local.operators, element(split(": ", pattern), 0)) ? join(": ", slice(split(": ", pattern), 1, length(split(": ", pattern)))) : pattern), new_value)]]]), contains(flatten([for p in values(local.compute_disks_tag_rules.update_values[key]) : p]), "else:") ? [format("      when new_key = '%s' then '%s'", key, [for new_value, patterns in local.compute_disks_tag_rules.update_values[key] : new_value if contains(patterns, "else:")][0])] : []]]))
}

locals {
  compute_disks_with_incorrect_tags_query = replace(
    replace(
      replace(
        replace(
          replace(
            replace(
              ...
              "__TABLE_NAME__", "azure_compute_disk"
            ),
            "__UPDATE_KEYS_OVERRIDE__", local.compute_disks_update_keys_override
          ),
          "__REMOVE_OVERRIDE__", local.compute_disks_remove_override
        ),
        "__REMOVE_EXCEPT_OVERRIDE__", local.compute_disks_remove_except_override
      ),
      "__ADD_OVERRIDE__", local.compute_disks_add_override
    ),
    "__UPDATE_VALUES_OVERRIDE__", local.compute_disks_update_values_override
  )
}