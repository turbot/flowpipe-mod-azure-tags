trigger "query" "detect_and_correct_kubernetes_clusters_with_incorrect_tags" {
  ...
  schedule = var.kubernetes_clusters_with_incorrect_tags_trigger_schedule
  database = var.database
  sql      = local.kubernetes_clusters_with_incorrect_tags_query
...
variable "kubernetes_clusters_tag_rules" {
  type = object({
...
locals {
  kubernetes_clusters_tag_rules = {
    ...
    update_values = merge(local.base_tag_rules.update_values, try(var.kubernetes_clusters_tag_rules.update_values, {}))
  }
}

locals {
  kubernetes_clusters_update_keys_override = join("\n", flatten([for key, patterns in local.kubernetes_clusters_tag_rules.update_keys : [for pattern in patterns : format("      when key %s '%s' then '%s'", (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? element(split(":", pattern), 0) : "="), (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? join(":", slice(split(":", pattern), 1, length(split(":", pattern)))) : pattern), key)]]))
  kubernetes_clusters_remove_override      = join("\n", length(local.kubernetes_clusters_tag_rules.remove) == 0 ? ["      when new_key like '%' then false"] : [for pattern in local.kubernetes_clusters_tag_rules.remove : format("      when new_key %s '%s' then true", (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? element(split(":", pattern), 0) : "="), (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? join(":", slice(split(":", pattern), 1, length(split(":", pattern)))) : pattern))])
  kubernetes_clusters_remove_except_override = join("\n", length(local.kubernetes_clusters_tag_rules.remove_except) == 0 ? ["      when new_key like '%' then true"] : flatten( [[for key in keys(merge(local.kubernetes_clusters_tag_rules.add, local.kubernetes_clusters_tag_rules.update_keys)) : format("      when new_key = '%s' then true", key)], [for pattern in local.kubernetes_clusters_tag_rules.remove_except : format("      when new_key %s '%s' then true", (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? element(split(":", pattern), 0) : "="), (length(split(":", pattern)) > 1 && contains(local.operators, element(split(":", pattern), 0)) ? join(":", slice(split(":", pattern), 1, length(split(":", pattern)))) : pattern))]]))
    kubernetes_clusters_add_override           = join(",\n", length(keys(local.kubernetes_clusters_tag_rules.add)) == 0 ? ["      (null, null)"] : [for key, value in local.kubernetes_clusters_tag_rules.add : format("      ('%s', '%s')", key, value)])
    kubernetes_clusters_update_values_override = join("\n", flatten([for key in sort(keys(local.kubernetes_clusters_tag_rules.update_values)) : [flatten([for new_value, patterns in local.kubernetes_clusters_tag_rules.update_values[key] : [contains(patterns, "else:") ? [] : [for pattern in patterns : format("      when new_key = '%s' and value %s '%s' then '%s'", key, (length(split(": ", pattern)) > 1 && contains(local.operators, element(split(": ", pattern), 0)) ? element(split(": ", pattern), 0) : "="), (length(split(": ", pattern)) > 1 && contains(local.operators, element(split(": ", pattern), 0)) ? join(": ", slice(split(": ", pattern), 1, length(split(": ", pattern)))) : pattern), new_value)]]]), contains(flatten([for p in values(local.kubernetes_clusters_tag_rules.update_values[key]) : p]), "else:") ? [format("      when new_key = '%s' then '%s'", key, [for new_value, patterns in local.kubernetes_clusters_tag_rules.update_values[key] : new_value if contains(patterns, "else:")][0])] : []]]))
    }

    locals {
      kubernetes_clusters_with_incorrect_tags_query = replace(
        replace(
          replace(
            replace(
              replace(
                replace(
                  ...
                  "__TABLE_NAME__", "azure_kubernetes_cluster"
                ),
                "__UPDATE_KEYS_OVERRIDE__", local.kubernetes_clusters_update_keys_override
              ),
              "__REMOVE_OVERRIDE__", local.kubernetes_clusters_remove_override
            ),
            "__REMOVE_EXCEPT_OVERRIDE__", local.kubernetes_clusters_remove_except_override
          ),
          "__ADD_OVERRIDE__", local.kubernetes_clusters_add_override
        ),
        "__UPDATE_VALUES_OVERRIDE__", local.kubernetes_clusters_update_values_override
      )
    }